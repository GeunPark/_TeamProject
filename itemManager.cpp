...
void itemManager::render()
{
	...
	//
	////실버
	//for (int i = 0; _vCoinSilver.size(); ++i)
	//{
	...
	//
	////브론즈
	//for (int i = 0; _vCoinBronze.size(); ++i)
	//{
	//	_vCoinSilver[i]->getItemImage()->frameRender(getMemDC(), _vCoinBronze[i]->getRect().left - _player->getPlayerCam().left, _vCoinBronze[i]->getRect().top - _player->getPlayerCam().top, _vCoinBronze[i]->getItemImage()->getFrameX(), _vCoinBronze[i]->getItemImage()->getFrameY());
	//}
