...
HRESULT enemyManager::init(void)
{
	...
	EFFECTMANAGER->addEffect("잠자리죽음", "잠자리dead", 0.2f, 10);
	EFFECTMANAGER->addEffect("나무인간죽음", "나무인간dead", 0.2f, 10);
	EFFECTMANAGER->addEffect("버그죽음", "버그dead", 0.2f, 10);
	EFFECTMANAGER->addEffect("통나무죽음", "통나무dead", 0.2f, 10);
	EFFECTMANAGER->addEffect("꼼장어죽음", "꼼장어dead", 0.2f, 10);
	EFFECTMANAGER->addEffect("두꺼비죽음", "두꺼비dead", 0.2f, 10);
...
void enemyManager::beAttackedEffect()
{
	for (int i = 0; i < _vEnemy.size(); ++i)
	{
		if (!_vEnemy[i]->getIsActived())
		{
			...
			{
			case DRAGONFLY:
				EFFECTMANAGER->play("잠자리죽음", _vEnemy[i]->getX(), _vEnemy[i]->getY());// - _player->getPlayerCam().left, _vEnemy[i]->getY() - _player->getPlayerCam().top);
				
				break;

			case VINEMAN:
				EFFECTMANAGER->play("나무인간죽음", _vEnemy[i]->getX(), _vEnemy[i]->getY());// - _player->getPlayerCam().left, _vEnemy[i]->getY() - _player->getPlayerCam().top);
			...
			case BUG:
				...
				EFFECTMANAGER->play("버그죽음", _vEnemy[i]->getX(), _vEnemy[i]->getY());// - _player->getPlayerCam().left, _vEnemy[i]->getY() - _player->getPlayerCam().top);

				break;

			case TREEMAN:
				EFFECTMANAGER->play("통나무죽음", _vEnemy[i]->getX(), _vEnemy[i]->getY());// - _player->getPlayerCam().left, _vEnemy[i]->getY() - _player->getPlayerCam().top);


				break;

			case ELECTRICEEL:
				EFFECTMANAGER->play("꼼장어죽음", _vEnemy[i]->getX(), _vEnemy[i]->getY());// - _player->getPlayerCam().left, _vEnemy[i]->getY() - _player->getPlayerCam().top);


				break;

			case PLANTFROG:
				EFFECTMANAGER->play("두꺼비죽음", _vEnemy[i]->getX(), _vEnemy[i]->getY());// - _player->getPlayerCam().left, _vEnemy[i]->getY() - _player->getPlayerCam().top);
